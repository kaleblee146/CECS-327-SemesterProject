...
# Label Encoder reference - https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.LabelEncoder.html
# PyTorch code referenced and modified from https://machinelearningmastery.com/training-a-linear-regression-model-in-pytorch/

class PredictData:
    def __init__(self): # creates instance of class
        self.data = None
        self.w = torch.tensor(2.0, requires_grad=True) # creates an instance of torch for deep learning, param 1
        self.b = torch.tensor(90.0, requires_grad=True) # param 2

    def import_data(self): # similar import data function from DataVisualizer.py
    ...
    def encode(self, column_name): # similar encoding function to convert
        label_encoder = LabelEncoder()
        self.data[column_name] = label_encoder.fit_transform(self.data[column_name])

    def encode_or_not(self, column_name): # similar encoding or not function 
        if self.data[column_name].dtype == 'object':
            self.encode(column_name)
        return pd.to_numeric(self.data[column_name], errors='coerce').fillna(0) # returns column as a int

    def forward(self, x): # forward function
        return self.w * x + self.b
    
    def criteron(self, y_pred, y): # criteron function
        return torch.mean((y_pred - y) ** 2)
    
    def create_plot(self): # create plot
        while True:
            ...
            for i in range(iterations): # loops through inputted iterations
            ...
            # plots the original data and the predicted line
    ...
    def saveFile(self): # save option function
        while True:
            try:
